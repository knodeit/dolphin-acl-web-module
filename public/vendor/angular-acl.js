...
angular.module('mm.acl').provider('AclService', [
    function () {
        ...
        var isStorage = function (storagetype) {
            var mod = 'dolphin_test';
            try {
                window[storagetype].setItem(mod, mod);
                window[storagetype].removeItem(mod);
                return true;
            } catch (e) {
                return false;
            }
        ...
        var saveToStorage = function (storagetype) {
            if (isStorage()) {
                ...
                setCookie(config.storageKey, data);
            }
        };
        ...
        var fetchFromStorage = function (storagetype) {
            var data;
            if (isStorage()) {
                data = window[storagetype].getItem(config.storageKey);
                data = (data) ? JSON.parse(data) : false;
            } else {
                data = getCookie(config.storageKey);
            }
            return data;
        };
